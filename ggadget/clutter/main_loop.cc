...
class MainLoop::Impl {
 ...
 public:
  Impl(MainLoopInterface *main_loop)
    ...
    ml_ = g_main_loop_new (NULL, FALSE);
  ...
  int AddIOWatch(MainLoopInterface::WatchType type, int fd,
    ...
    //DLOG causes deadlock here FIXME
    //fprintf(stderr, "MainLoop::AddIOWatch: id=%d fd=%d type=%d\n",
  ...
  int AddTimeoutWatch(int interval, WatchCallbackInterface *callback) {
    ...
    node->watch_id = watch_id;
    g_hash_table_insert(watches_, GINT_TO_POINTER(node->watch_id), node);
    //fprintf(stderr, "MainLoop::AddTimeoutWatch: id=%d interval=%d callback=%p\n",
    //     node->watch_id, node->data, callback);
    g_static_mutex_unlock(&mutex_);
    return watch_id;
  }
    ...
    if (!destroyed_) {
      ...
      if (node && !node->removing) {
        ...
        if (!node->calling) {
          ...
          //fprintf(stderr, "MainLoop::RemoveWatch: id=%d\n", watch_id);
          g_static_mutex_unlock(&mutex_);
  ...
  bool IsRunning() {
    return g_main_loop_is_running(ml_);
  }
 ...
 private:
  void RemoveWatchNode(WatchNode *node) {
    ...
    if (!node->removing) {
      ...
      WatchCallbackInterface *callback = node->callback;
      //fprintf(stderr, "MainLoop::RemoveWatchNode: id=%d\n", watch_id);
      g_static_mutex_unlock(&mutex_);
      callback->OnRemove(main_loop_, watch_id);
      g_static_mutex_lock(&mutex_);
      g_hash_table_remove(watches_, GINT_TO_POINTER(watch_id));
  ...
  static void NodeDestroyCallback(gpointer node) {
    //WatchNode *n = static_cast<WatchNode *>(node);
    //DLOG("Mainloop::NodeDestroyCallback: id=%d", n->watch_id);
    delete static_cast<WatchNode *>(node);
  }

  // Callback functions to be registered into glib's main loop for io watch.
  ...
  // Callback functions to be registered into glib's main loop for timeout
  // watch.
